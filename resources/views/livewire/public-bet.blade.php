        ...
        <div class="min-h-screen bg-gray-50">
            ...
            <div class="mx-auto -mt-8 max-w-7xl px-4">
                <div class="rounded-3xl bg-white p-6 shadow-2xl md:p-10">
                    @if (!$this->isVotingOpen)
                        <!-- Mensagem de Votação Encerrada -->
                        <div class="mb-6 rounded-xl border-2 border-red-300 bg-red-50 p-6 text-center">
                            <p class="text-lg font-bold text-red-700">
                                🔒 Votação Encerrada
                            </p>
                            <p class="mt-2 text-sm text-red-600">
                                A votação foi encerrada e não é mais possível enviar novos votos. Você pode consultar os
                                dados da votação usando seu código de acesso.
                            </p>
                            <a href="{{ route('check.bet') }}"
                                class="mt-4 inline-block rounded-lg bg-red-600 px-6 py-2 font-semibold text-white transition-colors hover:bg-red-700">
                                Consultar Dados
                            </a>
                        </div>
                    @else
                        <div>
                            <div class="mb-6 space-y-3">
                                <div class="rounded-xl border-2 border-blue-200 bg-blue-50 p-4">
                                    <p class="text-center text-sm font-semibold text-blue-800">
                                        ℹ️ Este é um sistema de votação colaborativa. Vote nos números que você acha que
                                        o
                                        grupo
                                ...
                                <div class="rounded-xl border-2 border-green-200 bg-green-50 p-4">
                                    <p class="text-center text-sm font-semibold text-green-800">
                                        ✓ Você pode votar sem enviar comprovante! O envio é opcional e pode ser feito
                                        depois.
                                    </p>
                                </div>
                            </div>
                            ...
                            @if (count($selectedNumbers) > 0)
                                <div class="mb-8">
                                    <div class="mb-4 flex items-center justify-between">
                                        ...
                                        <span
                                            class="rounded-full bg-green-100 px-4 py-1 text-sm font-bold text-green-700">
                            ...
                            @elseif (!$abstained)
                                <div
                                    class="mb-8 rounded-2xl bg-gradient-to-r from-green-50 to-emerald-50 p-6 text-center">
                                    <p class="font-medium text-green-700">
                                        👆 Escolha {{ $this->numbersToPickProperty }} números de
                                        {{ $this->minNumberProperty }}
                            ...
                            @else
                                <div
                                    ...
                                    <svg class="mx-auto mb-3 h-12 w-12 text-yellow-600" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M12 9v2m0 4v2m0 4v2M4.22 4.22a9 9 0 1112.56 12.56M4.22 4.22a9 9 0 0012.56 12.56" />
                                    </svg>
                                    <p class="text-lg font-bold text-yellow-900">Você escolheu se abster desta votação
                                    </p>
                                    <p class="mt-2 text-sm text-yellow-800">Nenhum número será selecionado para sua
                                        participação
                                    </p>
                                </div>
                            @endif
                            ...
                            <div class="mb-8">
                                ...
                                <div class="space-y-4">
                                    <div>
                                        <label class="mb-2 block text-sm font-medium text-gray-700">Nome Completo
                                            *</label>
                                    ...
                                    <div>
                                        ...
                                        <input type="tel" wire:model.live="phone"
                                            ...
                                            class="w-full rounded-lg border border-gray-300 px-4 py-3 transition-all focus:border-transparent focus:ring-2 focus:ring-green-500">
                                        <p class="mt-1 text-xs text-gray-500">Apenas se você quiser receber via
                                            WhatsApp
                                        </p>
                                        @error('phone')
                                            <span class="mt-1 text-xs text-red-500">{{ $message }}</span>
                                    ...
                                    <div class="rounded-lg border-2 border-yellow-200 bg-yellow-50 p-4">
                                        <label class="flex cursor-pointer items-center gap-3">
                                            ...
                                            <div>
                                                <p class="font-semibold text-yellow-900">Desejo me abster desta votação
                                                </p>
                                                <p class="text-xs text-yellow-800">Se marcar esta opção, você não
                                                    precisa
                                                    escolher números</p>
                                            </div>
                                        </label>
                                ...
                                <div class="mb-8">
                                    ...
                                    <div class="mb-4 rounded-xl border-2 border-blue-200 bg-blue-50 p-4">
                                        <p class="text-sm text-blue-800">
                                            ℹ️ Você pode enviar o comprovante agora ou depois. O envio não é obrigatório
                                            para
                                            votar.
                                        </p>
                                    </div>
                            ...
                            @if (!$abstained && count($selectedNumbers) < $this->numbersToPickProperty && count($selectedNumbers) > 0)
                                <p class="mt-4 text-center text-sm text-gray-500">
                                    Selecione mais {{ $this->numbersToPickProperty - count($selectedNumbers) }}
                                    número(s)
                                </p>
                            @endif
